...
import { SingleTextEdit, TextEdit, VirtualSourceDocument } from 'vs/editor/common/core/textEdit';
...
export class NativeEditContext extends AbstractEditContext {
	...
	private _handleTextFormatUpdate(e: TextFormatUpdateEvent): void {
		...
		const decorations: IModelDeltaDecoration[] = formats.map(f => {
			...
			const doc = new VirtualSourceDocument(lineNumber => this._context.viewModel.getLineContent(lineNumber), this._context.viewModel.getLineCount());
			const viewModelRange = this._editContextState!.viewModelToEditContextText.reverseMapRange(range, doc);
	...
	private updateText() {
		...
		const value = textEdit.applyToLines(doc);
...
class LineColLength {
	...
	public addTo(pos: Position): Position {
		if (this.lineCount === 0) {
			return pos.delta(0, this.columnCount);
		} else {
